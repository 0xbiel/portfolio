...
export default function Navbar() {
  ...
  const navLinks = (
    <>
      <Link href="/" onClick={() => setMenuOpen(false)} className="px-4 py-3">
        home
      </Link>
      ...
      <Link href="#links" onClick={() => setMenuOpen(false)} className="px-4 py-3">
        links
      </Link>
    </>
  );
  ...
  {/* Single floating navbar used both over the header and after scroll */}
        ...
        <div className="max-w-3xl mx-auto px-4 flex items-center justify-between  py-2">
          ...
          <div
            className={`text-xl font-bold text-[var(--main-color)] transition-all duration-300 ease-out transform ${
              isScrolledPastHeader ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-2 pointer-events-none'
            }`}
          >
            0xbiel
          </div>
          ...
          <div className={`hidden sm:flex items-center space-x-6 text-sm transition-all duration-300 ease-out ${
            isScrolledPastHeader ? 'justify-end w-auto' : 'justify-center w-full'
          }`}
          >
            <Link
              href="/"
      ...
      >
        ...
        <div className="mt-auto px-4 py-6 border-t border-neutral-100/10">
          <a
            href="mailto:hello@biel.codes"
            className="block w-full text-center rounded-md bg-[var(--main-color)]/10 px-4 py-2 text-sm text-[var(--main-color)]"
            onClick={() => setMenuOpen(false)}
          >
            Contact
          </a>
